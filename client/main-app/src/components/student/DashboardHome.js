    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-[#1E1E2E] p-8 rounded-3xl relative overflow-hidden border border-gray-800/50">
              <div className="relative z-10">
                  ...
                  <h1 className="text-3xl md:text-4xl font-bold text-white mt-4">
                      Welcome back, <span className="text-[#A78BFA]">{user?.user?.name || "Student"}!</span>
                  </h1>
          ...
          <div className="bg-[#E0Cffc] p-6 rounded-3xl flex flex-col justify-between text-[#2D1B4E]">
             <div className="flex justify-between items-start">
                 ...
                 <button><FiMoreHorizontal className="w-6 h-6" /></button>
             </div>
             <div>
      ...
      <div>
        <div className="flex justify-between items-center mb-6">
           <h2 className="text-xl font-bold text-white pl-4 border-l-4 border-purple-500">Active Courses</h2>
           <button className="bg-[#2B2B40] text-sm px-4 py-2 rounded-full hover:bg-purple-600 transition-colors">See All</button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
           {/* Card 1 */}
           {[1, 2, 3].map((item) => (
             <div key={item} className="bg-[#1E1E2E] p-5 rounded-3xl border border-gray-800 hover:border-purple-500/50 transition-all group">
                {/* Image Placeholder */}
                ...
                <h3 className="text-lg font-bold text-white mb-1">Learn UI/UX Design</h3>
                
                <div className="flex items-center gap-4 text-xs text-gray-400 mb-5 mt-3">
                    <span className="flex items-center gap-1 bg-[#2B2B40] px-2 py-1 rounded"><FiPlayCircle /> 35 Tutorials</span>
                    <span className="flex items-center gap-1 bg-[#2B2B40] px-2 py-1 rounded"><FiClock /> 40m/daily</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-[#1E1E2E] p-6 rounded-3xl border border-gray-800">
              <div className="flex justify-between mb-6">
                ...
                <select className="bg-[#2B2B40] text-xs px-3 py-1 rounded-lg outline-none border-none"><option>Courses</option></select>
              </div>
              <div className="flex items-end justify-between h-40 gap-4 px-2">
                 {[40, 70, 30, 85, 50, 90, 60].map((h, i) => (
                     <div key={i} className="w-full bg-[#2B2B40] rounded-t-lg h-full relative group">
                         <div style={{height: `${h}%`}} className="absolute bottom-0 w-full bg-linear-to-t from-purple-600 to-purple-400 rounded-t-lg transition-all duration-500 group-hover:from-purple-500 group-hover:to-purple-300"></div>
                     </div>
                 ))}
              </div>
          ...
          <div className="bg-[#1E1E2E] p-6 rounded-3xl border border-gray-800">
              ...
              <div className="space-y-4">
                  {[1, 2].map((i) => (
                      <div key={i} className="bg-[#2B2B40] p-3 rounded-2xl flex items-center gap-3">
              ...
              <button className="w-full mt-4 py-2 text-sm text-purple-400 hover:text-white transition-colors">See All</button>
          </div>
      </div>
